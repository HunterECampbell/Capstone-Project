---
title: "Pok&eacute;monGo Capstone Project"
...
cleanId <- read.csv(file = "C:/Users/hcnur_000/Desktop/Capstone Project/Capstone Project Machine Learning/cleanId.csv")
waterId <- read.csv(file = "C:/Users/hcnur_000/Desktop/Capstone Project/Capstone Project Machine Learning/waterId.csv")
notwaterId <- read.csv(file = "C:/Users/hcnur_000/Desktop/Capstone Project/Capstone Project Machine Learning/notwaterId.csv")
...
###**Machine Learning**
To go above and beyond a visual estimate of rarity, rarity can be calculated and then plotted again. In this way, any *Pok&eacute;monGo* user can have confidence between the difference of rare and common Pok&eacute;mon. In this way, all of the information above can be applied on a more successful scale.
...
The y-axis needs to be noted with this graph. Because there is a smaller amount of water type Pok&eacute;mon compared to all Pok&eacute;mon, the y-axis numbers will be less than the all Pok&eacute;mon types graph. Though, rarity is still calculated, specifically for water type Pok&eacute;mon, if compared with the all Pok&eacute;mon graph, the very common water type Pok&eacute;mon is just barely reaching the common cluster of the all Pok&eacute;mon graph and the common cluster of the water type Pok&eacute;mon graph is hovering about the same height as the rare cluster in the all Pok&eacute;mon graph. Even still, this graph is still acurately calculating rarity, but for water type Pok&eacute;mon only. So, if a *Pok&eacute;monGo* user is on the hunt for rare water type Pok&eacute;mon, this graph will be a good tool to help with a rare Pok&eacute;mon search.
...
**Comparison of Pok&eacute;mon Id to Machine Learning Pok&eacute;mon Rarity**
...
ggplot(cleanId, aes(pokemonId, freq, col = factor(cleanIdKMC$cluster), alpha = .5)) +
  geom_point() +
  labs(x = "All Pokemon Id's", y = "Frequency") +
  scale_color_manual(values = c("#800080", "#006622", "#e68a00", "#ff0000"),
                     name = "Rarity",
...
ggplot(waterId, aes(pokemonId, freq, col = factor(waterIdKMC$cluster), alpha = .5)) +
  geom_point() +
  labs(x = "Water Type Pokemon Id's", y = "Frequency") +
  scale_color_manual(values = c("#800080", "#e68a00", "#ff0000", "#006622"),
                     name = "Rarity",
...
ggplot(notwaterId, aes(pokemonId, freq, col = factor(notwaterIdKMC$cluster), alpha = .5)) +
  geom_point() +
  labs(x = "Non-water Type Pokemon Id's", y = "Frequency") +
  scale_color_manual(values = c("#ff0000", "#800080", "#e68a00", "#006622"),
                     name = "Rarity",
...
As each graph demonstrates, the K-means Algorithm calculated rarity under each case. It even calculated the rarity of water type Pok&eacute;mon; which means it can calculate any specific Pok&eacute;mon type, or even a group of Pok&eacute;mon (like the non-water type graph) with great accuracy. In this way, any *Pok&eacute;monGo* user can know which Pok&eacute;mon are rare, and which are common.
...
1. Analyze Pok&eacute;mon type to terrain type and their perspective, compared probabilites.

2. Calculate rarity for different Pok&eacute;mon types.

<br>

###**Recommendations**
1. While searching for rare Pok&eacute;mon, search near a gym and/or a pok&eacute;stop.

2. While searching for water type Pok&eacute;mon, search near water.

3. While searching for any type and/or rarity of Pok&eacute;mon, search in urban areas (Terrain Type: 13), during good weather, during the weekend, and/or during night time.
